...
export default function ActivityReport({ selectedDate, onCategorySelect }) {
  ...
  const incomeCategories = useMemo(() => {
    ...
    const active = Object.entries(map).map(([id, amount]) => ({ ...getCategoryById(id), amount, count: monthTransactions.filter(t => t.type === 'income' && t.categoryId === id).length })).sort((a,b) => b.amount - a.amount);
    return active.length ? active : INCOME_CATEGORIES.slice(0, 3).map(c => ({ ...c, amount: 0, count: 0 }));
  }, [monthTransactions]);
  ...
  return <div className="space-y-3">
    ...
    <section className="bg-card border border-border rounded-2xl p-3">
      <p className="text-sm font-bold text-text-primary">Tujuan pengeluaran</p>
      <div className="grid grid-cols-[minmax(145px,42%)_1fr] items-center gap-2 mt-3">
        <div className="relative w-full aspect-square max-w-[190px] justify-self-center"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={purposeData} dataKey="amount" nameKey="label" innerRadius="55%" outerRadius="82%" paddingAngle={2}>{purposeData.map(c => <Cell key={c.id} fill={c.color}/>)}</Pie><Tooltip formatter={(v) => formatCurrency(v)}/></PieChart></ResponsiveContainer><div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none"><span className="text-[10px] text-text-muted text-center">{dominantPurpose?.localLabel || 'Pengeluaran'}</span><span className="text-base font-bold text-text-primary">{purposeTotal && dominantPurpose ? `${(dominantPurpose.amount / purposeTotal * 100).toFixed(1)}%` : '0%'}</span></div></div>
        <div className="min-w-0 space-y-4">{purposeData.map(c => { const pct = purposeTotal ? c.amount / purposeTotal * 100 : 0; return <div key={c.id} className="min-w-0"><div className="grid grid-cols-[12px_minmax(0,1fr)_auto] items-start gap-2"><span className="w-3 h-3 rounded-full mt-1 shrink-0" style={{backgroundColor:c.color}}/><span className="text-sm text-text-secondary leading-tight">{c.localLabel}</span><span className="text-sm font-bold text-text-primary whitespace-nowrap">{formatShortCurrency(c.amount)}</span></div><div className="grid grid-cols-[12px_minmax(0,1fr)_auto] gap-2 mt-1.5"><span/><div className="h-1.5 bg-elevated rounded-full overflow-hidden"><div className="h-full rounded-full" style={{width:`${pct}%`, backgroundColor:c.color}}/></div><span className="text-[11px] text-text-muted whitespace-nowrap">{pct.toFixed(1)}%</span></div></div>; })}</div>
      </div>
    </section>
