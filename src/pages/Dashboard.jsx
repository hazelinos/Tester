  ...
  const transactionSubtitle = transactionPeriod === 'today'
    ...
    : transactionPeriod === '7days'
      ...
      : transactionPeriod === 'lastMonth'
        ...
        : `Aktivitas ${getMonthName(currentMonth.getMonth())} ${currentMonth.getFullYear()}`;

  const header = <header className="dashboard-header px-4 sm:px-7 pt-5 pb-4">
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="dashboard-marquee mt-0.5" aria-label="A Simple Money Tracker For Your Finance"><div className="dashboard-marquee-track">A Simple Money Tracker For Your Finance</div></div>
        </div>
      </div>
    </div>
  </header>;
    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-7 pt-4 sm:pt-6 space-y-5">
      <section className="dashboard-balance relative overflow-hidden rounded-[26px] p-5 sm:p-7">
        ...
        <div className="relative">
          <div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[.16em] text-primary/70">Total Uang</p><button onClick={() => setBalanceVisible((v) => !v)} className="dashboard-icon-button" aria-label="Tampilkan atau sembunyikan nominal">{balanceVisible ? <Eye size={17} /> : <EyeOff size={17} />}</button></div>
          <div className="flex items-end justify-between gap-4 mt-2"><p className="text-[30px] sm:text-[40px] leading-none font-extrabold tracking-tight text-white">{balanceVisible ? formatCurrency(balance) : 'Rp ••••••'}</p><span className="hidden sm:flex items-center gap-1.5 text-xs text-white/45"><WalletCards size={14} /> {accounts.length} akun</span></div>
      ...
      <section className="grid grid-cols-2 gap-3">
        <div className="dashboard-stat-card"><span className="dashboard-stat-icon income"><ArrowDownLeft size={17} /></span><span className="text-xs text-white/45">Pemasukan</span><strong className="text-lg sm:text-xl text-[#A8E6CF]">{formatShortCurrency(income)}</strong></div>
        <div className="dashboard-stat-card"><span className="dashboard-stat-icon expense"><ArrowUpRight size={17} /></span><span className="text-xs text-white/45">Pengeluaran</span><strong className="text-lg sm:text-xl text-[#FF8C8C]">{formatShortCurrency(expense)}</strong></div>
      </section>
      ...
      <section className="dashboard-glass-card p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
          <div><h2 className="text-base font-bold text-white">Transaksi</h2><p className="text-[11px] text-white/35 mt-0.5">{transactionSubtitle}</p></div>
          <div className="flex gap-1.5 overflow-x-auto scrollbar-none pb-0.5">
            {PERIOD_OPTIONS.map((option) => <button key={option.id} onClick={() => setTransactionPeriod(option.id)} className={clsx('shrink-0 rounded-xl px-3 py-2 text-[11px] font-semibold border transition-all', transactionPeriod === option.id ? 'bg-primary/15 border-primary/40 text-primary' : 'bg-white/[.025] border-white/8 text-white/45 hover:text-white/70')}>{option.label}</button>)}
          </div>
        </div>
        {filteredTransactions.length ? filteredTransactions.slice(0, mobile ? 5 : 8).map((tx) => <TransactionRow key={tx.id} transaction={tx} onEdit={openEdit} />) : <p className="py-8 text-center text-sm text-white/30">Belum ada transaksi untuk {periodLabel.toLowerCase()}</p>}
        {filteredTransactions.length > (mobile ? 5 : 8) && <Link to="/history" className="block text-center text-xs font-semibold text-primary pt-3">Lihat semua transaksi</Link>}
      </section>
