...
export default function Dashboard() {
  ...
  return <div className="dashboard-page min-h-full pb-24 sm:pb-8">
    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-7 pt-4 sm:pt-6 space-y-5">
      ...
      <section><div className="flex items-center gap-2 mb-3 px-0.5"><div className="flex-1 h-px bg-white/[.06]" /><button onClick={() => setEditingShortcut(true)} className="dashboard-edit-button" aria-label="Tambah shortcut"><Plus size={14} /> Tambah</button></div><div className="flex gap-2 overflow-x-auto scrollbar-none snap-x pb-1">{shortcuts.map((id) => { const option = SHORTCUT_OPTIONS.find((item) => item.id === id); return option ? <ShortcutCard key={id} option={option} savingsGoals={savingsGoals} budgetUsage={budgetUsage} overBudget={overBudget} /> : null; })}</div></section>
