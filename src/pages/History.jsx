  ...
  return (
    <div className="flex bg-elevated rounded-2xl p-1 gap-1">
      {tabs.map(({ id, label, Icon }) => (
        <button key={id} onClick={() => onChange(id)} className={clsx('flex-1 py-2.5 rounded-xl flex items-center justify-center gap-2 text-sm font-bold transition-all duration-200', active === id ? 'bg-card text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary')}>
...
function AdvancedFilter({ value, onChange, accounts, onClose }) {
  return (
    <div className="bg-card border border-border rounded-2xl p-3 space-y-3 shadow-lg">
      <div className="flex items-center justify-between"><p className="text-xs font-bold text-text-primary">Filter lanjutan</p><button onClick={onClose} className="text-text-muted"><X size={15} /></button></div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="text-[10px] text-text-muted">Kategori<select value={value.category} onChange={e => onChange({ ...value, category: e.target.value })} className="mt-1 w-full bg-input border border-border rounded-lg px-2 py-2 text-xs text-text-primary"><option value="all">Semua kategori</option>{ALL_CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</select></label>
...
function DetailSheet({ tx, accounts, onClose, onEdit, onDuplicate, onDelete }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}><div className="w-full max-w-lg bg-card border border-border rounded-t-3xl p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-2xl" onClick={e => e.stopPropagation()}><div className="mx-auto mb-4 h-1 w-10 rounded-full bg-border"/><div className="flex items-start gap-3"><div className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl" style={{backgroundColor:`${cat?.color||'#AAAAAA'}22`}}>{cat?.icon||'📦'}</div><div className="flex-1 min-w-0"><p className="font-bold text-text-primary truncate">{tx.note||cat?.label||'Transaksi'}</p><p className="text-xs text-text-muted mt-1">{cat?.label||'Lainnya'} · {isIncome?'Pemasukan':'Pengeluaran'}</p></div><p className={clsx('font-bold text-base',isIncome?'text-income':'text-expense')}>{isIncome?'+':'-'}{formatCurrency(tx.amount)}</p></div><div className="grid grid-cols-2 gap-2 mt-4">{[['Tanggal',date.toLocaleDateString('id-ID',{weekday:'long',day:'2-digit',month:'long',year:'numeric'})],['Waktu',time.toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'})],['Akun',account?.name||'—'],['Jenis transaksi',isIncome?'Pemasukan':'Pengeluaran']].map(([label,value])=><div key={label} className="bg-elevated rounded-xl p-3"><p className="text-[10px] text-text-muted">{label}</p><p className="text-xs font-semibold text-text-primary mt-1 break-words">{value}</p></div>)}</div><div className="bg-elevated rounded-xl p-3 mt-2"><p className="text-[10px] text-text-muted">Catatan</p><p className="text-sm text-text-primary mt-1">{tx.note||'Tidak ada catatan'}</p></div><div className="grid grid-cols-3 gap-2 mt-4"><button onClick={()=>onEdit(tx)} className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-elevated text-text-primary text-xs font-semibold"><Pencil size={14}/> Edit</button><button onClick={()=>onDuplicate(tx)} className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-elevated text-text-primary text-xs font-semibold"><Copy size={14}/> Duplikat</button><button onClick={()=>onDelete(tx)} className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-expense/10 text-expense text-xs font-semibold"><Trash2 size={14}/> Hapus</button></div></div></div>;
}
...
function CategoryPicker({ typeFilter, transactions, categoryFilter, onSelect }) {
  ...
  const categoryUsage = useMemo(() => { const counts = {}; transactions.forEach(tx => { if (tx.categoryId && (typeFilter === 'all' || tx.type === typeFilter)) counts[tx.categoryId] = (counts[tx.categoryId] || 0) + 1; }); return counts; }, [transactions, typeFilter]);
  const sortedCategories = useMemo(() => [...categories].sort((a,b) => { const usageDiff=(categoryUsage[b.id]||0)-(categoryUsage[a.id]||0); return usageDiff||categories.indexOf(a)-categories.indexOf(b); }), [categories, categoryUsage]);
  const rows = Math.min(3, Math.max(1, Math.ceil(sortedCategories.length / 5)));
  const columns = useMemo(() => { const result=[]; for(let i=0;i<sortedCategories.length;i+=rows) result.push(sortedCategories.slice(i,i+rows)); return result; }, [sortedCategories, rows]);
  return <div className="bg-card border border-border rounded-2xl p-2.5 space-y-2"><div className="flex items-center justify-between px-0.5"><div><p className="text-[11px] font-bold tracking-wide text-text-primary">Kategori</p><p className="text-[9px] text-text-muted">Diurutkan dari yang paling sering digunakan</p></div><span className="text-[9px] text-text-muted">{sortedCategories.length} kategori</span></div><div className="overflow-x-auto scrollbar-none -mx-0.5 px-0.5 pb-0.5 snap-x"><div className="flex gap-1.5 w-max">{columns.map((column,index)=><div key={index} className="grid gap-1.5 shrink-0" style={{gridTemplateRows:`repeat(${rows}, minmax(0, 1fr))`}}>{column.map((cat,catIndex)=>{const rank=index*rows+catIndex+1;const isSelected=categoryFilter===cat.id;return <button key={cat.id} type="button" onClick={()=>onSelect(cat.id)} className={clsx('w-[92px] h-[42px] rounded-xl border bg-input flex items-center gap-1.5 px-2 text-left transition-all shrink-0',isSelected?'border-primary bg-primary/10 text-primary':'border-border text-text-secondary')}><span className="text-[17px] leading-none shrink-0">{cat.icon}</span><span className="min-w-0 flex-1 text-[9px] leading-tight font-medium line-clamp-2">{cat.label}</span>{rank===1&&<span className="text-[8px] font-bold text-primary shrink-0">#1</span>}</button>})}</div>)}</div></div></div>;
}
