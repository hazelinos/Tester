...
export default function TransactionModal({ editTx, onClose, navigateToDebt }) {
  ...
  const transactionTimeLabel = getTimeLabel(editTx?.date);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 p-0 sm:p-3 modal-overlay">
      <div className="modal-sheet bg-card border border-border rounded-t-[24px] sm:rounded-2xl w-full max-w-md h-[100dvh] sm:h-[90vh] max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 border-t border-border bg-card px-4 pt-2.5 pb-[max(0.65rem,env(safe-area-inset-bottom))]"><div className="grid grid-cols-4 gap-1.5 select-none">
          <button type="button" onClick={()=>appendDigit('1')} className="keypad-key">1</button><button type="button" onClick={()=>appendDigit('2')} className="keypad-key">2</button><button type="button" onClick={()=>appendDigit('3')} className="keypad-key">3</button><button type="button" onClick={handleBackspace} className="keypad-key"><Delete size={19}/></button>
          <button type="button" onClick={()=>appendDigit('4')} className="keypad-key">4</button><button type="button" onClick={()=>appendDigit('5')} className="keypad-key">5</button><button type="button" onClick={()=>appendDigit('6')} className="keypad-key">6</button><button type="button" onClick={()=>appendOperator('+')} className="keypad-key">+</button>
          <button type="button" onClick={()=>appendDigit('7')} className="keypad-key">7</button><button type="button" onClick={()=>appendDigit('8')} className="keypad-key">8</button><button type="button" onClick={()=>appendDigit('9')} className="keypad-key">9</button><button type="button" onClick={()=>appendOperator('-')} className="keypad-key">−</button>
          <button type="button" onClick={appendDecimal} className="keypad-key">.</button><button type="button" onClick={()=>appendDigit('0')} className="keypad-key">0</button><button type="button" onClick={handleEquals} className="keypad-key">=</button><button type="button" onClick={()=>appendOperator('*')} className="keypad-key">×</button>
          <button type="button" onClick={handleClear} className="keypad-key col-span-2">C</button><button type="button" onClick={()=>appendOperator('/')} className="keypad-key">÷</button><button type="button" disabled={!isValid} onClick={handleSave} className="keypad-key keypad-save" aria-label={isEdit?'Simpan perubahan':'Simpan transaksi'}>✓</button>
        </div></div>
      </div>
    </div>
  );
