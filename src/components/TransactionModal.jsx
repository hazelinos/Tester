    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 p-0 sm:p-4 modal-overlay">
      <div className="modal-sheet bg-card border border-border rounded-t-[28px] sm:rounded-2xl w-full max-w-md h-[100dvh] sm:h-auto sm:max-h-[92vh] overflow-y-auto">
        ...
        <div className="px-5 pb-5 pt-2 space-y-4">
          ...
          <div><div className="flex items-center rounded-2xl border border-border bg-input overflow-hidden"><input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Deskripsi (Opsional)" maxLength={100} className="flex-1 bg-transparent px-4 py-4 text-base text-text-primary placeholder:text-text-muted focus:outline-none" /><span className="px-4 text-xl opacity-70">✨</span></div><p className="text-[11px] text-text-muted mt-2">✣ Otomatis kategorikan dari transaksi terakhir</p></div>
          ...
          {type === 'expense' && <div><p className="text-sm font-bold tracking-widest text-text-muted mb-2">TIPE</p><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setExpenseType('need')} className={clsx('rounded-2xl border px-4 py-3 text-center transition-all', expenseType === 'need' ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-input text-text-secondary')}><span className="block text-xl mb-1">🧺</span><span className="text-sm font-semibold">Kebutuhan</span></button><button type="button" onClick={() => setExpenseType('want')} className={clsx('rounded-2xl border px-4 py-3 text-center transition-all', expenseType === 'want' ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-input text-text-secondary')}><span className="block text-xl mb-1">👑</span><span className="text-sm font-semibold">Keinginan</span></button></div></div>}
