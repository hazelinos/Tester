...
export default function TransactionModal({ editTx, onClose }) {
  ...
  const changeType = (nextType) => {
    ...
    if (nextType === 'income') setExpenseType('need');
    setCategorySearch('');
  ...
  const appendDecimal = () => {
    setExpression((current) => {
      const base = justCalculated ? '' : current;
  ...
  const handleSave = () => {
    ...
    const data = {
      ...
      accountId,
      note: note.trim(),
      date: new Date(`${date}T${saveTime}:00`).toISOString(),
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 p-0 sm:p-3 modal-overlay">
      <div className="modal-sheet bg-card border border-border rounded-t-[24px] sm:rounded-2xl w-full max-w-md h-[100dvh] sm:h-[90vh] max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 pt-2 pb-1.5 sm:p-4 sm:border-b sm:border-border shrink-0">
          ...
          <div className="flex items-center gap-1">{isEdit && <button onClick={handleDelete} className="btn-danger px-2.5 py-1 text-xs flex items-center gap-1"><Trash2 size={13} /> Hapus</button>}<button onClick={onClose} className="p-1.5 rounded-lg hover:bg-elevated text-text-muted"><X size={17} /></button></div>
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto px-4 pt-1.5 pb-4">
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-1.5">
              <button type="button" onClick={() => changeType('expense')} className={clsx('py-2 border-b-2 text-sm font-bold', type === 'expense' ? 'text-expense border-expense' : 'text-text-muted border-transparent')}>PENGELUARAN</button>
              <button type="button" onClick={() => changeType('income')} className={clsx('py-2 border-b-2 text-sm font-bold', type === 'income' ? 'text-income border-income' : 'text-text-muted border-transparent')}>PEMASUKAN</button>
            </div>
        ...
        <div className="shrink-0 border-t border-border bg-card px-4 pt-2.5 pb-[max(0.65rem,env(safe-area-inset-bottom))]">
          <div className="grid grid-cols-4 gap-1.5 select-none">
            ...
            <button type="button" onClick={handleClear} className="keypad-key keypad-danger">C</button><button type="button" onClick={handleEquals} className="keypad-key keypad-blue">=</button><button type="button" onClick={()=>appendOperator('/')} className="keypad-key keypad-purple">÷</button><button type="button" disabled={!isValid} onClick={handleSave} className="keypad-key keypad-save" aria-label={isEdit?'Simpan perubahan':'Simpan transaksi'}>✓</button>
          </div>
        </div>
      </div>
    </div>
